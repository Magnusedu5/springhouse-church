'use client';
import Link from 'next/link';
import { motion } from 'framer-motion';
import { DriftIn, Reveal } from '@/components/motion';
import usePrefersReducedMotion from '@/lib/usePrefersReducedMotion';
import SiteLogo from '@/components/SiteLogo';

const quickLinks = [
  { href: '/', label: 'Home' },
  { href: '/about', label: 'About' },
  { href: '/sermons', label: 'Sermons' },
  { href: '/ministries', label: 'Ministries' },
  { href: '/events', label: 'Events' },
  { href: '/blog', label: 'Blog' },
  { href: '/contact', label: 'Contact' },
  { href: '/give', label: 'Give' },
];

const memberLinks = [
  { href: '/new-member', label: 'New Member Registration' },
  { href: '/member-registration', label: 'Existing Member Registration' },
];

function FacebookIcon() {
  return (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
    </svg>
  );
}

function InstagramIcon() {
  return (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      <path fillRule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" clipRule="evenodd" />
    </svg>
  );
}

function YouTubeIcon() {
  return (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      <path fillRule="evenodd" d="M19.812 5.418c.861.23 1.538.907 1.768 1.768C21.998 8.746 22 12 22 12s0 3.255-.418 4.814a2.504 2.504 0 01-1.768 1.768c-1.56.419-7.814.419-7.814.419s-6.255 0-7.814-.419a2.505 2.505 0 01-1.768-1.768C2 15.255 2 12 2 12s0-3.255.417-4.814a2.507 2.507 0 011.768-1.768C5.744 5 11.998 5 11.998 5s6.255 0 7.814.418zM15.194 12L10 15V9l5.194 3z" clipRule="evenodd" />
    </svg>
  );
}

export default function Footer() {
  const reduced = usePrefersReducedMotion();

  return (
    <footer className="bg-[#12202F] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-8">

          {/* Column 1 — Church Identity */}
          <DriftIn direction="left">
            <div>
              <motion.div
                initial={reduced ? false : { opacity: 0 }}
                whileInView={{ opacity: 1 }}
                viewport={{ once: true, margin: '0px 0px -10% 0px' }}
                transition={{ duration: 0.8, delay: reduced ? 0 : 1 }}
                className="mb-2"
              >
                <SiteLogo variant="footer" />
              </motion.div>
              <p className="font-display italic text-brand-gold text-lg mb-4">
                Across the street, across the seas!
              </p>
              <p className="text-white/70 text-sm leading-relaxed mb-6">
                To bring the lost to the saving knowledge of Christ and to mature believers
                who will minister across the seas.
              </p>
              <div className="flex gap-4">
                {/* TODO: Replace with real social profile URLs */}
                <motion.a
                  href="https://facebook.com"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="The SpringHouse Church on Facebook"
                  whileHover={reduced ? {} : { scale: 1.2 }}
                  transition={{ type: 'spring', stiffness: 300, damping: 15 }}
                  className="text-white/70 hover:text-brand-gold transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded"
                >
                  <FacebookIcon />
                </motion.a>
                <motion.a
                  href="https://instagram.com"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="The SpringHouse Church on Instagram"
                  whileHover={reduced ? {} : { scale: 1.2 }}
                  transition={{ type: 'spring', stiffness: 300, damping: 15 }}
                  className="text-white/70 hover:text-brand-gold transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded"
                >
                  <InstagramIcon />
                </motion.a>
                <motion.a
                  href="https://youtube.com"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="The SpringHouse Church on YouTube"
                  whileHover={reduced ? {} : { scale: 1.2 }}
                  transition={{ type: 'spring', stiffness: 300, damping: 15 }}
                  className="text-white/70 hover:text-brand-gold transition-colors focus:outline-none focus:ring-2 focus:ring-white rounded"
                >
                  <YouTubeIcon />
                </motion.a>
              </div>
            </div>
          </DriftIn>

          {/* Column 2 — Quick Links */}
          <Reveal>
            <div>
              <h3 className="text-xs font-semibold text-white uppercase tracking-widest mb-4">
                Quick Links
              </h3>
              <ul className="space-y-2.5" role="list">
                {quickLinks.map(({ href, label }) => (
                  <li key={href}>
                    <Link
                      href={href}
                      className="text-white/70 hover:text-white transition-colors text-sm focus:outline-none focus:underline"
                    >
                      {label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          </Reveal>

          {/* Column 3 — Join the Family */}
          <Reveal>
            <div>
              <h3 className="text-xs font-semibold text-white uppercase tracking-widest mb-4">
                Join the Family
              </h3>
              <ul className="space-y-2.5" role="list">
                {memberLinks.map(({ href, label }) => (
                  <li key={href}>
                    <Link
                      href={href}
                      className="text-brand-gold/80 hover:text-brand-gold transition-colors text-sm focus:outline-none focus:underline"
                    >
                      {label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          </Reveal>

          {/* Column 4 — Visit Us */}
          <DriftIn direction="right">
            <div>
              <h3 className="text-xs font-semibold text-white uppercase tracking-widest mb-4">
                Visit Us
              </h3>
              <address className="not-italic text-white/70 text-sm leading-relaxed space-y-3">
                <p>
                  137 Ndidem Usang Iso (Parliamentary Extension),<br />
                  Calabar, Cross River State, Nigeria
                </p>
                <p>
                  <span className="font-medium text-white">Sunday Service</span><br />
                  9:00 AM<br />
                  <span className="font-medium text-white">Midweek Service</span><br />
                  Wednesdays 5:00 PM
                </p>
                {/* TODO: Add phone number */}
                <p>
                  <span className="font-medium text-white">Phone:</span> TBC
                </p>
                {/* TODO: Add email address */}
                <p>
                  <span className="font-medium text-white">Email:</span> TBC
                </p>
              </address>
            </div>
          </DriftIn>

        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-brand-gold/30">
        <p className="text-center text-white/50 text-xs py-5 px-4">
          © {new Date().getFullYear()} The SpringHouse Church. All rights reserved.
        </p>
      </div>
    </footer>
  );
}
